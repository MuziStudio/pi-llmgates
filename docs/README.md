...
| 文档 | 说明 | 未落地的部分 |
| --- | --- | --- |
| [2026-09-22-model-audit-design.md](./superpowers/specs/2026-09-22-model-audit-design.md) | 上游响应模型不一致审计（`/model-audit`）：在插件 provider 的 `stream` 层注入 `fetch` 旁路读取上游自报模型（pi-ai ≥0.83 三种接口），系列不同才计数；状态行 Turn / All 追加红色 `.xN`；按工作目录保存最多 300 条；root marker 带所有者规则与 Turn 快照，写入单一历史文件并由父会话轮询。文末「实施记录与偏差」逐条列出与方案的差异 | §10.2 真实安装包门禁一项未跑：pi-subagents / pi-subagents-lite 前后台、worktree cwd、`/new` `/resume` `/reload`、占锁退出时长，以及 0.81.0 floor / 0.86.0 upper-bound |
| [2026-09-06-all-subagent-live-usage-design.md](./superpowers/specs/2026-09-06-all-subagent-live-usage-design.md) | 全子代理准实时用量方案：统一账本、来源身份、self/subtree 去重、origin-turn 与 idle 更新；核对本机 pi-subagents 0.66.0 / Pi 0.85.1、15 个第三方包及三组外部 CLI。附 [固定版本源码与目录发现清单](./superpowers/specs/2026-09-06-all-subagent-live-usage-inventory.json)（341 个目录匹配项只是发现结果，不是支持声明） | child factory / 全树观测、经 runtime fixture 认证的第三方逐响应、CLI JSONL（Pi 0.85/0.86 已进 peer 范围但仍未 certified，见 S0 修订）。probe 与 Coverage `unavailable` **不得**接进 All |
| [2026-09-07-usage-s0-freeze.md](./superpowers/specs/2026-09-07-usage-s0-freeze.md) | S0 冻结：`llmgates:usage:v1` 字段与质量规则、开关名称/默认值、存储限额、peer 上界（**0.7.1 已按该文件原定条件修订为 `<0.87.0`**，见文首修订说明）。实施计划见 [2026-09-07-all-subagent-live-usage.md](./superpowers/plans/2026-09-07-all-subagent-live-usage.md) | 同上：factory、certified fixture、CLI JSONL（0.86.0 已进 peer 范围，certified 仍为 false） |
