...
另有三个与网关注册失败隔离、默认开启的功能：输入历史跨进程持久化（`/input-history`）、新会话恢复上次
使用的模型与思考档位（`restoreLastModel`），以及上游响应模型审计（`/model-audit`，观察本插件 provider
的请求，只记录不拦截）。三者都在网关注册之前单独注册、不受网关注册失败连坐（`extensions/index.ts`）。
